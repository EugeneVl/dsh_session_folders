...
function apply(ctx) {
	return ctx.storageDomain.open(foldersDomainSpec).then((foldersDomain) => {
		...
		*/
		...
		/** Open a workspace folder in the system file manager. The harness's
		 * host.openPath RPC currently returns ok without navigating for some
		 * paths, so we spawn the file manager directly (verified: direct
		 * explorer spawn opens the correct folder). */
		route("open-folder", async (body, res) => {
			...
			let stats;
			try {
				stats = await import("node:fs").then((m) => m.promises.stat(target));
			} catch {
				return respond(res, 404, { error: "path-not-found" });
			}
			if (!stats.isDirectory()) return respond(res, 400, { error: "not-a-directory" });
			try {
				...
				child.unref();
				respond(res, 200, { opened: true });
			} catch (error) {
				respond(res, 502, { error: "open-failed", message: String(error?.message ?? error ?? "failed to open") });
			}
		});
		...
		route("unarchive", async (body, res) => {
			...
			// skipping on the stale set.
			const workspace = ctx.storageDomain.get("workspace");
			if (workspace === void 0) return respond(res, 500, { error: "workspace-domain-unavailable" });
