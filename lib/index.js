		...
		*/
		...
		route("reorder-folders", async (body, res) => {
			...
			return withMutationLock(async () => {
				...
				if (orderedIds.length !== owned.length ||
					orderedIds.some((id) => !ownedIds.has(id) || (seen.has(id) ? true : (seen.add(id), false)))) {
					return respond(res, 400, { error: "bad-request" });
				}
		...
		route("reorder-workspaces", async (body, res) => {
			...
			return withMutationLock(async () => {
				...
				const seen = new Set();
				if (orderedIds.length !== live.length ||
					orderedIds.some((id) => !liveSet.has(id) || (seen.has(id) ? true : (seen.add(id), false)))) {
					return respond(res, 400, { error: "bad-request" });
				}
				const record = readRecord();
