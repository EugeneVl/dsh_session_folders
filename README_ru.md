...
- **Автопереименование**: в контекстном меню сессии — «Auto rename»: модель самой сессии читает её первое сообщение и придумывает краткое название из 3–4 слов (описание процесса/фичи/задачи, на языке сообщения); результат закрепляется как обычное ручное переименование
...
6. **Автопереименование** — правый клик по сессии → «Auto rename»: модель сессии по первому сообщению придумывает название из 3–4 слов. В простое ничего не вызывает; ошибки — в notice-бар
