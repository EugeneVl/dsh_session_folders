...
- **Свернуть / развернуть всё**: пара кнопок-шевронов в шапке браузера сворачивает и разворачивает все группы воркспейсов и папки в один клик
...
- **Секция Recent**: над списком воркспейсов — пять самых свежих сессий воркспейсов (из папок или свободной зоны), новые сверху; клик открывает сессию в чате, и она подсвечивается и в Recent, и в своём воркспейсе/папке. Заголовок сворачивает/разворачивает секцию (состояние сохраняется и следует за «Свернуть всё / Развернуть всё»)
- **Блок Archive**: иконка архива на строке воркспейса показывает/скрывает виртуальную папку Archive со всеми заархивированными сессиями воркспейса; пока показана, иконка перечёркнута. Открытие папки также разворачивает её; внутри сессии идут от новых к старым (сначала пять, со строкой «Показать ещё N / Показать меньше», как в оригинальном браузере). Перетаскивание: сброс сессии на неё архивирует сессию (как пункт меню); сброс заархивированной сессии на папку или в свободную зону восстанавливает её там
- **Восстановление из архива**: правый клик по заархивированной сессии — «Восстановить в исходную папку» (сессия возвращается туда, где была до архивации, или в свободную зону); левый клик восстанавливает её в папку **Restored** воркспейса (создаётся по требованию) и открывает в чате. Восстановленные сессии отделены от обычных: папка Restored всегда первая (сразу под блоком Archive) и прячется, когда в ней нет видимых сессий
...
2. **Создать папку** — правый клик по строке воркспейса → «Новая папка»; имя должно быть уникальным в рамках воркспейса
3. **Переместить сессию** — перетащите строку сессии на папку (только в папку того же воркспейса) или правый клик по сессии → «Переместить в папку…» → выберите папку или «Новая папка…», чтобы создать и переместить сразу
4. **Вернуть в свободную зону** — правый клик по сессии → «Переместить в папку…» → «Workspace» (первый пункт)
5. **Закрепить сессию** — правый клик → «Pin»: сессия встаёт в начало своей папки (или свободной зоны) и остаётся там, пока приходят другие сессии; «Unpin» возвращает порядок «новые сверху». У закреплённой сессии без бейджа статуса в слоте статуса — маленькая булавка
6. **Переименовать / удалить папку** — правый клик по строке папки; удаление требует подтверждения, сессии папки становятся свободными
7. **Поиск** — поле вверху браузера; совпадения подсвечены и кликабельны
8. **Свернуть / развернуть всё** — пара кнопок-шевронов вверху браузера:
   - свернуть: сворачивает все группы воркспейсов и папки (включая Recent и Archive)
   - развернуть: снова раскрывает их
9. **Порядок** — перетащите строку воркспейса на новое место; строку папки — в пределах её воркспейса (верхняя/нижняя половина строки = до/после)
10. **Открыть папку воркспейса** — первая кнопка (иконка папки) на строке воркспейса открывает корневой каталог в системном файловом менеджере
11. **Новая сессия** — «+» на строке воркспейса создаёт сессию в нём; «+» поменьше на строке папки — сразу внутри папки
12. **Архив** — иконка архива на строке воркспейса показывает/скрывает виртуальную папку Archive (все заархивированные сессии воркспейса, новые сверху, по пять за раз). Сброс сессии на неё архивирует; сброс заархивированной сессии на папку или в свободную зону восстанавливает
13. **Recent** — секция над списком воркспейсов показывает пять самых свежих сессий (папки + свободная зона). Клик открывает сессию; строка в Recent и иконки воркспейса/папки отмечают текущую сессию; клик по сессии в Recent раскрывает свёрнутые воркспейс/папку. Заголовок сворачивает секцию
14. **Восстановление** — правый клик по заархивированной сессии → «Восстановить в исходную папку» (на прежнее место); левый клик восстанавливает в папку **Restored** (если свёрнута — раскроется автоматически) и открывает сессию. Папка Restored всегда первая и прячется, когда пуста
15. **Быстрая архивация** — наведите на строку сессии: метка времени на её месте сменится иконкой архива; клик — архивация

## Как это устроено

| Слой | Реализация |
|---|---|
| Хост | `lib/index.js` — cordis-плагин: 9 POST-роутов `/dsh-session-folders/{list,create,rename,delete,move,reorder-folders,reorder-workspaces,pin,unarchive}`; собственный storage-домен `dsh_session_folders` (одна глобальная запись со списком папок); мутации сериализуются через promise-хвост, чтобы два браузера не перезаписывали друг друга; воркспейсы и принадлежность сессий валидируются через `ctx.workspaceRegistry` |
